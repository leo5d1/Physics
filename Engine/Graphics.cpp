...
void Graphics::DrawLine(const glm::ivec2& start, const glm::ivec2& end, glm::vec4 color)
{
    SetColor(color);
    SDL_RenderDrawLine(m_renderer, start.x, start.y, end.x, end.y);
}

void Graphics::DrawCircle(const glm::ivec2& point, const float radius, const glm::vec4 color)
{
    SetColor(color);
    circleRGBA(m_renderer, point.x, point.y, radius, color.r, color.g, color.b, color.a);
}

void Graphics::DrawFilledCircle(const glm::ivec2& point, const float radius, const glm::vec4 color)
{
    SetColor(color);
    filledCircleRGBA(m_renderer, point.x, point.y, radius, color.r, color.g, color.b, color.a);
}

glm::ivec4 Graphics::ConvertColor(glm::vec4& color)
{
    glm::ivec4 icolor;
    icolor.r = (int)(color.r * 255);
    icolor.g = (int)(color.g * 255);
